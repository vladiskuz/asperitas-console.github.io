...
![Создать снимок](../../../images/user_guide/12_instances_make_snapshot.png)
...
2.  Выберите соответствующий проект из мменю переключения проектов в верхнем левом углу.
...
![Плавающий IP](../../../images/user_guide/13_instances_floating_ip.png)
...
5.  На странице `Создание образа` введите следующие значения:
    ...
    `Формат диска`: Выберите формат образа (например, QCOW2) для образа.

    `Формат контейнера`: Для виртуальных машин выбирайте Bare. Для образов Docker, выбирайте Docker (но это нужно только если вы точно знаете, что в системе есть узлы, использующие контейнеры вместо виртуальных машин, иначе в этом нет смысла).

    `Операционная система`: Выберите операционную систему образа (например, CentOS).

    `Версия ОС`: Версия операционной системы образа. Используйте общепринятую нотацию; система обращает внимание на номера версий.

    `Администратор ОС`: Имя администратора операционной системы образа. В общем случае, Administrator для Windows, root для Linux.
    ...
    `Дополнительные опции`: Имеет смысл менять только если вы точно знаете, что делаете.

    ![Создание образа](../../../images/user_guide/14_instances_images.png)

6.  Нажмите `Подтвердить`.

    Образ ставится в очередь на загрузку. Может потребоваться некоторое время, прежде чем статус изменится с `В очереди` на `Активный`.
